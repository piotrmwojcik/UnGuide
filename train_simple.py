...
def main():
    ...
    hyper_train_steps = config.get(
        "hyper_train_steps", 500
    )  # Steps for hypernetwork context
    ...
    retain_csv_path = config.get(
        "retain_csv_path", None
    )  # Path to CSV with retain prompts

    # Augmentation flags
    augment_target = config.get(
        "augment_target", True
    )  # Whether to augment target concepts
    augment_retain = config.get(
        "augment_retain", False
    )  # Whether to augment retain prompts from CSV
    ...
    internal_lr = config.get(
        "internal_lr", 1e-4
    )  # Simulated lr for hypernetwork gradient matching
    ...
    for layer_name, layer in hyper_lora_layers:
        ...
        accelerator.unwrap_model(model).hyper.add_hyperlora(
            layer_name, layer.hyper_lora
        )
    ...
    if retain_csv_path and os.path.exists(retain_csv_path):
        ...
        if "prompt" not in df.columns:
            raise ValueError(
                f"CSV file must have a 'prompt' column. Found columns: {df.columns.tolist()}"
            )
                ...
                for prompt in tqdm(retain_prompts, desc="Creating retain embeddings"):
                    ...
                    with torch.no_grad():
                        if use_pooler:
                            ...
                            emb = clip_text_encoder(inputs).last_hidden_state.detach()
                    retain_embeddings.append(
                        emb.squeeze().cpu()
                    )  # Store on CPU for caching
    ...
    for iteration in pbar:
        ...
        with accelerator.accumulate(model):
            ...
            with torch.no_grad():
                ...
                emb_p = base.get_learned_conditioning(
                    [target_text_augmented]
                )  # target prompt (positive)
                emb_n = base.get_learned_conditioning(
                    [target_text_augmented]
                )  # target prompt (negative, to be erased)
                emb_m = base.get_learned_conditioning(
                    [mapping_text_augmented]
                )  # mapping prompt (what target should map to)

            # Random timestep for HyperLoRA context
            rtimestep = int(
                torch.randint(0, hyper_train_steps - 1, (1,), device=accelerator.device)
            )
            base.hyper.set_context(
                target_emb, torch.tensor([rtimestep], device=accelerator.device)
            )
            ...
            with torch.no_grad():
                ...
                e_m = model_orig.apply_model(
                    z, t_enc_ddpm, emb_m
                )  # mapping (reference) concept
            ...
            accelerator.backward(
                loss_aux / accelerator.gradient_accumulation_steps, retain_graph=True
            )
            ...
            accelerator.backward(
                loss_remove / accelerator.gradient_accumulation_steps
            )  # retain_graph = True
            # we must delete the graph at the end of the iteration

            if len(retain_embeddings) > 0:
                # Sample multiple retain concepts
                num_retain_samples = min(10, len(retain_embeddings))
                sampled_retain_embs = random.sample(
                    retain_embeddings, num_retain_samples
                )

                # Batch process retain concepts
                batch_retain_embs = torch.stack(sampled_retain_embs, dim=0).to(
                    accelerator.device
                )

                hyper = base.hyper
                batch_prompts = batch_retain_embs.repeat(
                    hyper_train_steps // num_retain_samples, 1
                )
            ...
            for diag_idx, diag_prompt in enumerate(diagnostic_prompts):
                ...
                with torch.no_grad():
                    if use_pooler:
                        ...
                        diag_emb = clip_text_encoder(
                            inputs_diag
                        ).last_hidden_state.detach()

                base.hyper.set_context(
                    diag_emb,
                    torch.tensor([hyper_train_steps], device=accelerator.device),
                )
                ...
                combined_model = CombinedCFGModel(
                    cond_model=base, uncond_model=model_orig
                ).eval()
                    ...
                    wandb.log(
                        {
                            f"diagnostic_{diag_idx}_{safe_key}": wandb.Image(
                                to_pil_image(im0), caption=diag_prompt
                            )
                        },
                        step=iteration,
                    )
