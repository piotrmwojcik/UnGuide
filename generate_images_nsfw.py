...
from generate_images import AutoGuidedModel, decide_w, load_model_from_config
...
if __name__ == "__main__":
    ...
    print(
        "images", len(os.listdir(os.path.join(exp_dirpath, "images"))), flush=True
    )
    ...
    with torch.no_grad():
        for k, v in lora_sd.items():
            if k in sd:
                if (
                    torch.is_tensor(lora_sd[k])
                    and torch.is_tensor(v)
                    and lora_sd[k].shape == v.shape
                ):
    ...
    for image_id, row in df.iterrows():
        ...
        start_code = torch.randn(
            (1, 4, args.image_size // 8, args.image_size // 8), device=model.device
        )
        ...
        model.hyper.compute_and_cache_loras(
            t_prompt, torch.tensor([150]).to(model.device)
        )
        ...
        print(
            f"Prompt [{prompt}] processed in {end - start:.2f}) seconds. Saved to {image_path}",
            flush=True,
        )
